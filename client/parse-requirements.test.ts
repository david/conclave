...
function makeBlock(useCases: UseCase[]): string {
  return "```conclave:requirements\n" + JSON.stringify(useCases, null, 2) + "\n```";
}

describe("parseRequirements", () => {
  test("extracts use cases from a tagged code block", () => {
    const markdown = "Here is the analysis:\n\n" + makeBlock([uc1]) + "\n\nThat's all.";
    expect(parseRequirements(markdown)).toEqual([uc1]);
  });

  test("extracts multiple use cases", () => {
    const markdown = makeBlock([uc1, uc2]);
  ...
  test("handles multiple tagged blocks and merges results", () => {
  ...
  test("ignores incomplete block (no closing fence)", () => {
