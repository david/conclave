...
describe("AssistantMarkdown invalid block fallback", () => {
...
describe("AssistantMarkdown isReplay prop", () => {
  ...
  test("next-block button is enabled when isReplay is false", () => {
    const html = renderToStaticMarkup(
      <AssistantMarkdown text={nextBlockMarkdown} isReplay={false} />,
    );
  ...
  test("next-block button is disabled when isReplay is true", () => {
    const html = renderToStaticMarkup(
      <AssistantMarkdown text={nextBlockMarkdown} isReplay={true} />,
    );
    expect(html).toContain("next-block__diamond");
  ...
  test("next-block button is enabled when isReplay is omitted", () => {
    const html = renderToStaticMarkup(
      <AssistantMarkdown text={nextBlockMarkdown} />,
    );
    expect(html).toContain("next-block__diamond");
    expect(html).not.toContain("next-block--disabled");
  });
...
describe("AssistantMarkdown heading normalization", () => {
  test("heading immediately after text on same line is rendered as a heading", () => {
    const html = renderToStaticMarkup(<AssistantMarkdown text="Some text.### Heading" />);
    expect(html).toContain("<h3");
    expect(html).toContain("Heading");
  });

  test("heading with existing preceding newline is unchanged", () => {
    const html = renderToStaticMarkup(<AssistantMarkdown text={"Some text.\n\n### Heading"} />);
    expect(html).toContain("<h3");
    expect(html).toContain("Heading");
  });

  test("heading at start of text is unchanged", () => {
    const html = renderToStaticMarkup(<AssistantMarkdown text={"### Heading\n\nSome text."} />);
    expect(html).toContain("<h3");
    expect(html).toContain("Heading");
  });

  test("multiple heading levels without preceding newlines are all normalized", () => {
    const html = renderToStaticMarkup(<AssistantMarkdown text="Text.## H2 more.### H3" />);
    expect(html).toContain("<h2");
    expect(html).toContain("<h3");
  });

  test("hash characters in non-heading context are not affected", () => {
    const html = renderToStaticMarkup(<AssistantMarkdown text="Issue #123 is fixed" />);
  ...
  test("heading syntax inside inline code is not split onto its own line", () => {
    const html = renderToStaticMarkup(
      <AssistantMarkdown text={'mentions `## Wave N` in inline code'} />,
    );
  ...
  test("multiple inline code spans with heading syntax are preserved", () => {
    const html = renderToStaticMarkup(
      <AssistantMarkdown text={'Use `## H2` or `### H3` for headings'} />,
    );
    expect(html).not.toContain("<h2");
    expect(html).not.toContain("<h3");
...
describe("AssistantMarkdown code fence separation", () => {
...
describe("AssistantMarkdown markdown rendering", () => {
  test("renders bold, italic, lists, links, code blocks", () => {
    const html = renderToStaticMarkup(
      <AssistantMarkdown text="**bold** *italic* [link](http://x.com)\n\n- item" />,
    );
...
describe("AssistantMarkdown does not apply remark-breaks", () => {
  test("single newline is not a br", () => {
    const html = renderToStaticMarkup(
      <AssistantMarkdown text="line one\nline two" />,
    );
